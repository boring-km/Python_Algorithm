...
def getMinMax(tree_min, node, start, end, left, right):
    global mm
    ...
    # 이도 저도 아닌경우
    mid = (start+end) >> 1
    getMinMax(tree_min, node << 1, start, mid, left, right)
    getMinMax(tree_min, (node << 1) + 1, mid + 1, end, left, right)
...
for i in range(M):
    ...
    getMinMax(mintree, 1, 0, N-1, left-1, right-1)
    print(mm)